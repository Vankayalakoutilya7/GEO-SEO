...
try:
    ...
    print("WARNING: Required packages (reportlab) not installed. PDF generation will be disabled.")
...
def generate_report(data, output_path="GEO-REPORT.pdf"):
    ...
    if "-" in date:
        try:
            # Fix: only parse the date part if HH:MM is present
            date_part = date.split(" ")[0] if " " in date else date
            display_date = datetime.strptime(date_part, "%Y-%m-%d").strftime("%B %d, %Y")
        except:
            pass

    details_data = [
        ["Website", url],
        ["Analysis Date", display_date],
    ...
    score_data = [
        ...
        ["Content Similarity (Echo Penalty)", f"{data.get('metrics', {}).get('echo_penalty', 0)}%", "Mod", "N/A"],
    ...
    if findings:
        for finding in findings:
            ...
            if description:
                # Split description into points for better readability
                lines = description.split("\n")
                render_lines = []
                for line in lines:
                    line = line.strip()
                    if not line: continue
                    # Detect if it's already a bullet or reasoning point
                    if line.startswith(("+", "-", "•", "*", "1.", "2.", "3.")):
                        render_lines.append(f"• {line.lstrip('+-•*123456789. ')}")
                    else:
